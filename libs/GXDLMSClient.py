...
from libs.GXDLMSSettings import GXDLMSSettings
from gurux_dlms.enums import Authentication, InterfaceType, SourceDiagnostic, DataType, Conformance
from gurux_dlms.ConnectionState import ConnectionState
from gurux_dlms.GXByteBuffer import GXByteBuffer
from gurux_dlms.GXHdlcSettings import GXHdlcSettings
from gurux_dlms.enums import Command, ObjectType
...
from gurux_dlms.ActionRequestType import ActionRequestType
from gurux_dlms.GetCommandType import GetCommandType
from libs._GXCommon import _GXCommon
from gurux_dlms.objects import GXDLMSObject, GXDLMSObjectCollection, GXDLMSData
from gurux_dlms.internal._GXDataInfo import _GXDataInfo
from libs.ValueEventArgs import ValueEventArgs
from libs.GXDateTime import GXDateTime
from gurux_dlms.SetRequestType import SetRequestType
from gurux_dlms.GXDLMSSNParameters import GXDLMSSNParameters
from gurux_dlms.VariableAccessSpecification import VariableAccessSpecification
from gurux_dlms.GXSecure import GXSecure
from gurux_dlms.GXDLMSConverter import GXDLMSConverter
from gurux_dlms.GXDLMSLNCommandHandler import GXDLMSLNCommandHandler
from gurux_dlms.GXDLMSSNCommandHandler import GXDLMSSNCommandHandler
from gurux_dlms.enums.AccessServiceCommandType import AccessServiceCommandType
from gurux_dlms.enums.ErrorCode import ErrorCode
from gurux_dlms.GXDLMSTranslatorStructure import GXDLMSTranslatorStructure
from gurux_dlms.enums.RequestTypes import RequestTypes
from gurux_dlms.SerialnumberCounter import SerialNumberCounter
...
class GXDLMSClient(object):
    ...
    increaseInvocationCounterForGMacAuthentication = property(getIncreaseInvocationCounterForGMacAuthentication,
    ...
    def parseAareResponse(self, reply):
        self.isAuthenticationRequired = _GXAPDU.parsePDU(self.settings, self.settings.cipher, reply,
                                                         None) == SourceDiagnostic.AUTHENTICATION_REQUIRED
    ...
    def getApplicationAssociationRequest(self):
        ...
        if self.manufacturerId == "LGZ" and self.settings.authentication == Authentication.HIGH:
            challenge = self.encryptLandisGyrHighLevelAuthentication(self.settings.password,
                                                                     self.settings.stoCChallenge)
            if self.useLogicalNameReferencing:
                return self.__method("0.0.40.0.0.255", ObjectType.ASSOCIATION_LOGICAL_NAME, 1, challenge,
        ...
        challenge = GXSecure.secure(self.settings, self.settings.cipher, self.settings.cipher.invocationCounter,
                                    self.settings.getStoCChallenge(), pw)
        if self.useLogicalNameReferencing:
            return self.__method("0.0.40.0.0.255", ObjectType.ASSOCIATION_LOGICAL_NAME, 1, challenge,
                                 DataType.OCTET_STRING)
        return self.__method(0xFA00, ObjectType.ASSOCIATION_SHORT_NAME, 8, challenge, DataType.OCTET_STRING)
    ...
    def releaseRequest(self):
        ...
        if self.useLogicalNameReferencing:
            ...
            reply = GXDLMS.getSnMessages(
                GXDLMSSNParameters(self.settings, Command.RELEASE_REQUEST, 0xFF, 0xFF, None, buff))
        self.settings.connected = self.settings.connected & ~ConnectionState.DLMS
        return reply
    ...
    def __method(self, name, objectType, methodIndex, value, dataType=DataType.NONE):
        ...
        if self.useLogicalNameReferencing:
            ...
            p = GXDLMSLNParameters(self.settings, 0, Command.METHOD_REQUEST, ActionRequestType.NORMAL,
            ...
            if type_ != DataType.NONE:
                ...
                attributeDescriptor.setUInt8(0)
            p = GXDLMSSNParameters(self.settings, Command.WRITE_REQUEST, 1, VariableAccessSpecification.VARIABLE_NAME,
                                   attributeDescriptor, data)
            reply = GXDLMS.getSnMessages(p)
        return reply

    def write(self, item, index, offset=None):
        e = ValueEventArgs(self.settings, item, index, 0, None)
        if offset is None:
            value = item.getValue(self.settings, e)
        else:
            try:
                value = item.getValue(self.settings, e, offset)
            except:
                value = item.getValue(self.settings, e)
        ...
        return self.__write(item.name, value, type_, item.objectType, index, offset)

    def write_with_type(self, item, index, type_):
        e = ValueEventArgs(self.settings, item, index, 0, None)
        value = item.getValue(self.settings, e)
        if type_ == DataType.OCTET_STRING and isinstance(value, (str,)):
            ui = item.getUIDataType(index)
    ...
    def __write(self, name, value, dataType, objectType, index, offset=None):
        # pylint: disable=bad-option-value,redefined-variable-type
        if index < 1:
            raise Exception("Invalid parameter")
        self.settings.resetBlockIndex()
        ...
        if self.useLogicalNameReferencing:
            ...
            attributeDescriptor.setUInt8(0)
            p = GXDLMSLNParameters(self.settings, 0, Command.SET_REQUEST, SetRequestType.NORMAL, attributeDescriptor,
                                   data, 0xff)
    ...
    def _read(self, name, objectType, attributeOrdinal, data=None):
        ...
        if self.useLogicalNameReferencing:
            ...
            if not data:
                ...
                attributeDescriptor.setUInt8(1)
            p = GXDLMSLNParameters(self.settings, 0, Command.GET_REQUEST, GetCommandType.NORMAL, attributeDescriptor,
                                   data, 0xFF)
            reply = GXDLMS.getLnMessages(p)
        else:
            # pylint: disable=bad-option-value,redefined-variable-type
        ...
        if columns:
            ...
            for c in columns:
                ...
                for k, v in pg.captureObjects:
                    pos += 1
                    if k.objectType == k.objectType and k.logicalName == c[0].logicalName and v.attributeIndex == c[
                        1].attributeIndex and v.dataIndex == c[1].dataIndex:
                        found = True
                        if pos < columnIndex:
                            columnIndex = pos
    ...
    def readRowsByRange(self, pg, start, end, columns=None):
        ...
        if not columns:
            ...
            for it in columns:
                ...
                _GXCommon.setData(self.settings, buff, DataType.OCTET_STRING,
    ...
    def readRowsIndexToIndex(self, pg, index, index_last, columns=None):
        if index < 0:
            raise ValueError("index")
        if index_last < 0:
            raise ValueError("index_last")
        pg.buffer = list()
        buff = GXByteBuffer(19)
        buff.setUInt8(0x02)
        buff.setUInt8(DataType.STRUCTURE)
        buff.setUInt8(0x04)
        _GXCommon.setData(self.settings, buff, DataType.UINT32, index)
        _GXCommon.setData(self.settings, buff, DataType.UINT32, index_last)
        columnIndex = 1
        columnCount = 0
        pos = 0
        if columns:
    ...
    def getData(self, reply, data, notify=None):
        ...
        if ret and self.translator and data.moreData == RequestTypes.NONE:
            if data.xml is None:
                data.xml = (GXDLMSTranslatorStructure(self.translator.outputType, self.translator.omitXmlNameSpace,
                                                      self.translator.hex, self.translator.showStringAsHex,
                                                      self.translator.comments, self.translator.tags))
            ...
            try:
                ...
                if data.command == Command.SNRM or data.command == Command.UA:
                    ...
                    if data.data.size != 0:
                        ...
                        self.translator._pduToXml2(data.xml, data.data, self.translator.omitXmlDeclaration,
                                                   self.translator.omitXmlNameSpace, True)
