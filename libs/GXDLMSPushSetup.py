...
from gurux_dlms.objects.enums import ServiceType, MessageType


# pylint: disable=too-many-instance-attributes
class GXDLMSPushSetup(GXDLMSObject):
    """
    Online help:
    ...
    def __init__(self, ln="0.7.25.9.0.255", sn=0):
        ...
        self.pushInterface = '0.2.22.0.0.255'
        self.clientAddress = 40
        self.lastConfirmation = None
        self.methodOfConfirmation = 2
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            ret = _GXCommon.logicalNameToBytes(self.pushInterface)
        elif e.index == 9:
            ret = self.clientAddress
        elif e.index == 11:
            ret = self.methodOfConfirmation
        elif e.index == 13:
            ret = self.lastConfirmation
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
        return ret
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if e.value:
                ...
                try:
                    if self.service == ServiceType.HDLC:
                        ...
                        if self.destination and not GXByteBuffer.isAsciiString(e.value[1]):
            ...
            self.pushInterface = _GXCommon.toLogicalName(e.value)
        elif e.index == 9:
            self.clientAddress = e.value
        elif e.index == 11:
            self.methodOfConfirmation = e.value
        elif e.index == 13:
            if isinstance(e.value, bytearray):
                self.lastConfirmation = _GXCommon.changeType(settings, e.value, DataType.DATETIME)
            else:
                self.lastConfirmation = e.value
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
    ...
    def load(self, reader):
        ...
        self.pushInterface = reader.readElementContentAsInt("PushInterface")
        self.clientAddress = reader.readElementContentAsInt("ClientAddress")
        self.methodOfConfirmation = reader.readElementContentAsInt("MethodOfConfirmation")
        self.lastConfirmation = reader.readElementContentAsInt("LastConfirmation")
    ...
    def save(self, writer):
        ...
        writer.writeElementString("PushInterface", self.pushInterface)
