...
from gurux_dlms.enums import ClockStatus, DateTimeExtraInfo, DateTimeSkips, DataType
from gurux_dlms.GXTimeZone import GXTimeZone
...
class GXDateTime:
    def __init__(self, value=None, pattern=None):
        ...
        if isinstance(value, datetime.datetime):
            if value.tzinfo is None:
                ...
                self.value = datetime.datetime(value.year, value.month, value.day, value.hour, value.minute,
                                               value.second, 0, tzinfo=tz)
            else:
                self.value = value
        ...
        if value:
            ...
            if addTimeZone:
                ...
                if tz is None:
                    ...
                    self.skip |= DateTimeSkips.DEVITATION
                else:
                    tmp = datetime.datetime(tmp.year, tmp.month, tmp.day, tmp.hour, tmp.minute, tmp.second, 0,
    ...
    def toHex(self, addSpace, useMeterTimeZone):
        ...
        buff = GXByteBuffer()
        settings = GXDLMSSettings(False, 0)
