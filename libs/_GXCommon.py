...
from gurux_dlms import GXByteBuffer, GXDate, GXArray, GXStructure, GXTimeZone, GXFloat64, GXFloat32, GXEnum, \
    GXUInt64, GXUInt16, TranslatorTags, GXUInt8, GXInt8, GXUInt32, GXBitString, GXInt16, GXInt64, GXInt32
from gurux_dlms.enums import DataType, DateTimeSkips, DateTimeExtraInfo, TranslatorOutputType, ClockStatus, Standard
from gurux_dlms.internal._GXDataInfo import _GXDataInfo
from pytz import unicode
from six import unichr

from libs.GXDateTime import GXDateTime
from libs.GXTime import GXTime
...
class _GXCommon:
    ...
    def changeType(cls, settings, value, type_):
        ...
        if value is None:
            ...
            ret = GXByteBuffer(value)
        elif type_ == DataType.DATETIME and isinstance(value, GXDateTime):
            ret = value
        elif type_ == DataType.DATE and isinstance(value, GXDate):
            ret = value
        elif type_ == DataType.TIME and isinstance(value, GXTime):
            ret = value
    ...
    def getString(cls, buff, info, knownType):
        ...
        if info.xml:
            if info.xml.showStringAsHex:
                info.xml.appendLine(info.xml.getDataType(info.type_), None,
    ...
    #
    @classmethod
    def setData(cls, settings, buff, dataType, value, offset=None):
        ...
        if dataType == DataType.NONE:
            ...
            if isinstance(value, GXDate):
                #  Add size
                buff.setUInt8(5)
                cls.setDate(buff, value, offset)
            elif isinstance(value, GXTime):
                #  Add size
                buff.setUInt8(4)
                cls.setTime(buff, value, offset)
            elif isinstance(value, (GXDateTime, datetime)):
                buff.setUInt8(12)
                cls.setDateTime(settings, buff, value, offset)
            else:
    ...
    #
    @classmethod
    def setTime(cls, buff, value, offset):
        ...
        if dt.skip & DateTimeSkips.HOUR != DateTimeSkips.NONE:
            ...
            if offset is not None:
                buff.setUInt8(dt.value.hour + offset.hour)
            else:
        ...
        if dt.skip & DateTimeSkips.MINUTE != DateTimeSkips.NONE:
            ...
            if offset is not None:
                buff.setUInt8(dt.value.minute + offset.minute)
            else:
        ...
        if dt.skip & DateTimeSkips.SECOND != DateTimeSkips.NONE:
            ...
            if offset is not None:
                buff.setUInt8(dt.value.second + offset.second)
            else:
    ...
    #
    @classmethod
    def setDate(cls, buff, value, offset):
        dt = _GXCommon.__getDateTime(value)
        #  Add year.
        if dt.skip & DateTimeSkips.YEAR != DateTimeSkips.NONE:
            ...
            if offset is not None:
                buff.setUInt16(dt.value.year+offset.year)
            else:
        ...
        if dt.extra & DateTimeExtraInfo.DST_BEGIN != 0:
            ...
            if offset is not None:
                buff.setUInt8(dt.value.month + offset.month)
            else:
        ...
        if dt.extra & DateTimeExtraInfo.LAST_DAY2 != DateTimeSkips.NONE:
            ...
            if offset is not None:
                buff.setUInt8(dt.value.day + offset.day)
            else:
    ...
    #
    @classmethod
    def setDateTime(cls, settings, buff, value, offset=None):
        ...
        if skip & DateTimeSkips.MILLISECOND != DateTimeSkips.NONE:
            ...
            if offset is not None:
                ms = dt.value.microsecond + offset.microsecond
            else:
                ms = dt.value.microsecond
    ...
    def toLogicalName(cls, value):
        if isinstance(value, bytearray):
            if not value:
                value = bytearray(6)
            if len(value) == 6:
                return str(value[0]) + "." + str(value[1]) + "." + str(value[2]) + "." + str(value[3]) + "." + str(
                    value[4]) + "." + str(value[5])
    ...
    def dlmsSystemTitleToString(cls, st):
        ...
        sb += _GXCommon.__getChar(st[0]) + _GXCommon.__getChar(st[1]) + _GXCommon.__getChar(st[2])
        sb += "Serial number: "
        sb += cls.__getChar(st[3]) + cls.__getChar(st[4]) + cls.__getChar(st[5]) + cls.__getChar(st[6]) + cls.__getChar(
            st[7])
        return sb
